...
async function sendEmail({ to, cc, subject, html, text, replyTo }) {
  ...
  const payload = { from, to, cc, subject, html, text, replyTo };
  ...
  if (private_env.RESEND_API_KEY) {
    try {
      const res = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({ from, to, cc, subject, html, text, reply_to: replyTo })
      });
      if (!res.ok) {
  ...
  console.log("[email:fallback] (no SMTP or RESEND configured) would send:", { from, to, cc, subject });
  return { ok: true, mocked: true, transport: "console" };
}
async function sendToUserAndAdmin({ userEmail, userName, subject, body }) {
...
//# sourceMappingURL=validation-DEofG8tH.js.map
